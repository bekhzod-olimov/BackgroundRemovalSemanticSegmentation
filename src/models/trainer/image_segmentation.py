...
class ImageSegmentation(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for idx, (image, mask) in enumerate(zip(batch['image'], batch['mask'])):
            ...
            total_score += f1_score(side_outputs[0].detach().cpu(), mask.cpu())
        ...
        dice_coff = {'dice_coff': total_score / len(loss_list)}
        self.log_dict(dice_coff)
        self.dice_coff = dice_coff
        return val_loss
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        checkpoint['dice_coff'] = self.dice_coff
